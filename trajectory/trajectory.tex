...
Topics:
\begin{itemize}
    \item Trigonometry, inverse trigonometry
    \item Vector algebra
    \item Algebra
    \item Physics, kinematics
\end{itemize}

Difficulty: at least \textit{AP Physics C: Mechanics} level.

\subsection*{Math Preface}
...
\begin{enumerate}
    \item Define variables: givens, unknowns, useful definitions
    \item Use simple intuition or physics to create an equation with the variables
    \item Perform many algebraic operations on the equation one-by-one to change it into a more useful equation
    \item The final equation reveals new information, such as an expression for an unknown
\end{enumerate}
...
We are shooting at a target. The target is either the \textit{hub} when the robot is inside the alliance zone, or the \textit{alliance zone} (via the trench or bump) when the robot is inside the neutral zone.

The \textit{trajectory} of the \textit{projectile} (fuel) starts at the \textit{muzzle} (the point in space where the projectile exits the shooter), and ends at the target. For the duration of the trajectory, the projectile is only influenced by gravity (it is 'ballistic').

A \textit{reference frame} is like a perspective. Imagine two cars moving in the same forward direction, Car A at 30 mph and Car B at 40 mph. From the reference frame (perspective) of Car A, Car B is moving forward away at 10mph. From the reference frame of Car B, Car A is moving backwards at 10mph. \\
...
The default coordinate system used for vectors is the field coordinate system (blue origin), abbreviated \textbf{FCS}. Just think of it as an unchanging, static, or absolute system. \\
The robot coordinate system, abbreviated \textbf{RCS}, is relative to the robot. Vectors with this coordinate system are "attached" to or follow the robot's reference frame; its position and rotation. An example is the locations of the robot wheels relative to the robot.
...
\begin{itemize}
    \item $\alpha$: alpha (lowercase)
    \item $\theta$: theta (lowercase)
    \item $\phi$: phi (lowercase)
    \item $\sigma$: sigma (lowercase)
    \item $\Delta$: Delta (uppercase) - means "difference" or "change [in]"
\end{itemize}
...
\subsection*{Vectors}
A \textit{vector} is like a list of components. Each component corresponds to a coordinate. 2D vectors have $x$ and $y$ coordinate components, 3D vectors additionally have a $z$ component. \\
A regular number is called a \textit{scalar}.
...
A \textit{unit vector} is a vector with magnitude $1$. They have a hat on top instead of an arrow, e.g. $\hat{w}$.
...
These can transform vectors and other rotations via multiplication. The rotation $\boldsymbol{R}$ is \textit{applied} to the other object.
...
\paragraph*{Forward Vector}
The \textit{forward vector} of $\boldsymbol{R}$ is a unit vector pointing in the direction that an object with rotation $\boldsymbol{R}$ is facing.
...
\subsubsection*{Projectile Exit Speed}
...
\subsubsection*{Vector Kinematics Equation}
...
\subsubsection*{Eliminate Other Unknowns $\alpha$, $\theta$}
...
\subsubsection*{Convert to Scalar Quartic}
...
\begin{itemize}
    \item $V_x^2+V_y^2=\lVert\vec{V}\rVert^2$
    \item $S_x^2+S_y^2+S_z^2=\lVert\vec{S}\rVert^2$
    \item $S_xV_x+S_yV_y=S_xV_x+S_yV_y+(S_z\cdot0)=\vec{S}\cdot\vec{V}$
\end{itemize}
...
\subsection*{3. Find $\alpha$ and $\theta$}
...
The situation where the robot moves while the target is still is \textbf{perfectly analogous} to the situation where the robot is still while the target moves (with opposite velocity). \\
...
\subsubsection*{Calculating $\alpha$ from $t$}
...
\begin{align*}
    ...
    (\text{use definition of }\angle)\implies&\arctan\left(\frac{u\cos\theta\sin\alpha}{u\cos\theta\cos\alpha}\right)=\angle\vec{L} \\
    (\text{simplify fraction})\implies&\arctan(\tan\alpha)=\angle\vec{L} \\
    (\text{inverse functions cancel})\implies&\boxed{\alpha=\angle\vec{L}}
\end{align*}

\subsubsection*{Calculating $\theta$ from $t$}
...
Next, for the $xy$ (horizontal) component, considered together, use the velocity definition. Note that $\lVert\hat{w}_{xy}\rVert=\cos\theta$.
\[ \hookrightarrow\vec{v}_{xy}=\frac{\vec{S}_{xy}}{t} \]
\begin{align*}
    ...
    (\text{use }\,\lVert\hat{w}_{xy}\rVert=\cos\theta\,) \implies&ut\cos\theta=\lVert\vec{L}\rVert
\end{align*}
...
\subsection*{4. Convert to Robot Coordinate System}
...
\begin{align*}
    &\hookrightarrow\theta_r=\text{pitch}(\boldsymbol{R}_S)=\arcsin\left(\frac{-d_z}{\lVert\hat{d}\rVert}\right) \\
...
\end{align*}
(Due to things with the reference frame of 3D rotations, the $z$ component is the \textit{negated} $\sin$ of the pitch angle.)
...
\[ \left(\frac{g^2}{4}\right)t^4+(S_zg+\lVert\vec{V}\rVert^2-u^2)t^2-2(\vec{S}\cdot\vec{V})t+\lVert\vec{S}\rVert^2=0 \]
...
\section*{Extra Case: If $\theta$ is a Parameter}
...
\begin{align*}
    ...
    (\text{take arcsin of sides})\implies&\alpha-\phi=\arcsin\left(\frac{\vec{V}\times\vec{S}}{\sigma\lVert\vec{S}\rVert}\right) \\
    (\text{move }\phi\text{ to other side})\implies&\alpha=\phi+\arcsin\left(\frac{\vec{V}\times\vec{S}}{\sigma\lVert\vec{S}\rVert}\right) \\
\end{align*} 
...
\[ \boxed{\alpha=\angle\vec{S}+\arcsin\left(\frac{\vec{V}\times\vec{S}}{u\cos(\theta)\lVert\vec{S}\rVert}\right)} \]
...
\subsection*{Invalid Trajectories}
Some trajectories are physically impossible because of obstacles.

Primarily, the hub walls require steep trajectories.

One way to validate the trajectory is to check that the end of the trajectory is at a steep enough downwards angle. First, take $\vec{v}_{xyz}$. Then calculate final velocity $\vec{v}_f=\vec{v}+\vec{a}t$
...
\subsection*{Reducing and Compensating for Error}
